            ...
            return (
              <div key={req.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <button onClick={() => setExpanded(open ? null : req.id)}
                  ...
                  <div className="w-11 h-11 rounded-full flex items-center justify-center text-white shrink-0" style={{ background: 'linear-gradient(135deg, #94a3b8, #64748b)' }}>
                    <IconUser size={20} />
                  </div>
                ...
                {open && (
                  <div className="px-4 pb-4 animate-fade-in">
                    <p className="text-xs font-medium text-slate-500 mb-2">Rol tanlang:</p>
                    <div className="grid grid-cols-2 gap-2">
                      {ROLES.map((r) => (
                        <button key={r.key} onClick={() => approve.mutate({ id: req.id, role: r.key })}
                          disabled={approve.isPending}
                          className="press py-2.5 px-3 bg-red-50 text-red-600 rounded-xl text-xs font-semibold disabled:opacity-50">
                          {r.label}
                        </button>
                      ))}
                    </div>
