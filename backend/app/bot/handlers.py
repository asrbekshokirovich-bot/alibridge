import logging

from aiogram import F, Router
from aiogram.filters import Command, CommandStart
from aiogram.fsm.context import FSMContext
from aiogram.fsm.state import State, StatesGroup
from aiogram.types import (
    InlineKeyboardButton,
    InlineKeyboardMarkup,
    KeyboardButton,
    Message,
    ReplyKeyboardMarkup,
    WebAppInfo,
)
from sqlalchemy import select

from app.core.config import settings
from app.core.enums import Role, StaffRequestStatus
from app.db.base import SessionLocal
from app.db.models import StaffRequest, User

logger = logging.getLogger("alibridge.bot")

router = Router()


class Reg(StatesGroup):
    phone = State()


def _miniapp_keyboard() -> InlineKeyboardMarkup:
    url = settings.miniapp_url or "https://example.com"
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="📦 ALI BRIDGE — Ilovani ochish",
                    web_app=WebAppInfo(url=url),
                )
            ]
        ]
    )


def _phone_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="📱 Raqamni ulashish", request_contact=True)]],
        resize_keyboard=True,
        one_time_keyboard=True,
        input_field_placeholder="Pastdagi tugmani bosing...",
    )


async def _get_user(telegram_id: int) -> User | None:
    async with SessionLocal() as db:
        return await db.scalar(select(User).where(User.telegram_id == telegram_id))


@router.message(CommandStart())
async def cmd_start(message: Message, state: FSMContext) -> None:
    if not message.from_user:
        return
    tg = message.from_user
    logger.info(
        "START bosildi: telegram_id=%s name=%s username=@%s",
        tg.id,
        tg.first_name,
        tg.username,
    )

    user = await _get_user(tg.id)
    if user is not None:
        await state.clear()
        await message.answer(
            f"👋 Xush kelibsiz, <b>{user.first_name}</b>!\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "📦 <b>ALI BRIDGE</b> — yuk logistikasi tizimi\n"
            "━━━━━━━━━━━━━━━━━━━━\n\n"
            "Ilovani ochish uchun pastdagi tugmani bosing 👇",
            reply_markup=_miniapp_keyboard(),
            parse_mode="HTML",
        )
        return

    await state.set_state(Reg.phone)
    await message.answer(
        f"👋 Assalomu alaykum, <b>{tg.first_name}</b>!\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "📦 <b>ALI BRIDGE</b>\n"
        "🇺🇿 Toshkent → 🇹🇷 Turkiya yuk yetkazish\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Ro'yxatdan o'tish uchun telefon raqamingizni ulashing.\n"
        "Pastdagi tugmani bosing 👇",
        reply_markup=_phone_keyboard(),
        parse_mode="HTML",
    )


@router.message(Reg.phone, F.contact)
async def reg_phone(message: Message, state: FSMContext) -> None:
    contact = message.contact
    tg = message.from_user
    if contact.user_id != tg.id:
        await message.answer(
            "⚠️ Iltimos, <b>o'z</b> raqamingizni ulashing.\n\n"
            "Pastdagi tugmani bosing 👇",
            reply_markup=_phone_keyboard(),
            parse_mode="HTML",
        )
        return

    phone = contact.phone_number
    if not phone.startswith("+"):
        phone = "+" + phone

    first_name = tg.first_name or ""
    last_name = tg.last_name or ""
    await state.clear()

    async with SessionLocal() as db:
        existing = await db.scalar(select(User).where(User.telegram_id == tg.id))
        if existing is None:
            db.add(
                User(
                    telegram_id=tg.id,
                    first_name=first_name,
                    last_name=last_name,
                    phone=phone,
                    role=Role.NEW,
                    is_active=True,
                )
            )
            await db.commit()

    await message.answer(
        f"🎉 <b>Tabriklaymiz, {first_name}!</b>\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "✅ Ro'yxatdan o'tish muvaffaqiyatli yakunlandi!\n"
        f"📱 Raqam: <code>{phone}</code>\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Endi ilovani ochib, o'zingizga mos bo'limni tanlang 👇",
        reply_markup=_miniapp_keyboard(),
        parse_mode="HTML",
    )


@router.message(Reg.phone)
async def reg_phone_invalid(message: Message) -> None:
    await message.answer(
        "⚠️ Telefon raqami qabul qilinmadi.\n\n"
        "Pastdagi <b>📱 Raqamni ulashish</b> tugmasini bosing 👇",
        reply_markup=_phone_keyboard(),
        parse_mode="HTML",
    )


@router.message(F.text == "/app")
async def cmd_app(message: Message) -> None:
    await message.answer(
        "📦 <b>ALI BRIDGE</b> — ilovani ochish uchun:",
        reply_markup=_miniapp_keyboard(),
        parse_mode="HTML",
    )


@router.message(Command("ishchi"))
async def cmd_ishchi(message: Message) -> None:
    if not message.from_user:
        return
    tg = message.from_user

    async with SessionLocal() as db:
        user = await db.scalar(select(User).where(User.telegram_id == tg.id))

        if user is None:
            await message.answer(
                "⚠️ Siz hali ro'yxatdan o'tmagansiz.\n\n"
                "Boshlash uchun /start ni bosing.",
                parse_mode="HTML",
            )
            return

        staff_roles = {
            Role.WAREHOUSE_UZ,
            Role.WAREHOUSE_TR,
            Role.COURIER_UZ,
            Role.COURIER_TR,
            Role.ADMIN,
        }
        if user.role in staff_roles:
            await message.answer(
                "✅ Siz allaqachon <b>xodim</b> sifatida tizimda ro'yxatdansiz.\n\n"
                "Ilovaga kirish uchun /app ni bosing.",
                parse_mode="HTML",
            )
            return

        existing = await db.scalar(
            select(StaffRequest).where(
                StaffRequest.user_id == user.id,
                StaffRequest.status == StaffRequestStatus.PENDING,
            )
        )
        if existing is not None:
            await message.answer(
                "⏳ <b>So'rovingiz ko'rib chiqilmoqda.</b>\n\n"
                "Admin tasdiqlashini kuting. Tez orada javob keladi.",
                parse_mode="HTML",
            )
            return

        user.role = Role.PENDING
        db.add(StaffRequest(user_id=user.id, status=StaffRequestStatus.PENDING))
        await db.commit()

    await message.answer(
        "✅ <b>So'rov yuborildi!</b>\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "👤 Xodim bo'lish uchun arizangiz qabul qilindi.\n"
        "⏳ Admin ko'rib chiqib, rol tayinlaydi.\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Tasdiqlangach sizga xabar yuboriladi. 🔔",
        parse_mode="HTML",
    )

    from app.bot.notify import on_staff_request

    async with SessionLocal() as db:
        await on_staff_request(db, name=f"{tg.first_name} {tg.last_name or ''}".strip())
