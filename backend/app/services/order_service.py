...
async def confirm_order_item(
    ...
    *,
    ...
    if product.type == ProductType.WEIGHT and actual_kg is None:
        raise AppError("ACTUAL_KG_REQUIRED", "Tekstil uchun kg kiritish shart")

    item.actual_quantity = actual_quantity
    item.actual_kg = actual_kg if product.type == ProductType.WEIGHT else None
